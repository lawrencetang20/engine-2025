...
// GAME PARAMETERS
static const int numRounds = 1000 static const int startingStack = 400;
static const int bigBlind = 2 static const int smallBlind = 1;
...
struct Bot
{
  ...
  */
  ...
  Action getPreflopAction(GameInfoPtr gameState, int active)
  {
  ...
  */
  ...
  std::pair<Action, int> getPostflopAction(GameInfoPtr gameState, int active)
  {
  ...
  Action getAction(GameInfoPtr gameState, RoundStatePtr roundState, int active)
  {
    ...
    if (street == 0)
    {
      return getPreflopAction(gameState, active);
    }
    else{
      std::pair<Action, int> postflopAction = getPostflopAction(gameState, active);
    }
